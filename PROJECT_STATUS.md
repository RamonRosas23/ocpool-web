...
- **Integración de servidor:** `main` conserva el dominio `ocpool.com.mx`, el correo `contacto@ocpool.com.mx`, el arranque local en `127.0.0.1:3008` y la ausencia del endpoint legado `/api/send-email`. El commit local está sincronizado con `origin/main`, pero el worktree conserva cambios sin commit (`62` archivos rastreados modificados y `31` archivos nuevos); cualquier publicación requiere primero revisar, agrupar y confirmar esos cambios, y no equivale a despliegue productivo.
