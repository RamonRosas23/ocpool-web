...
/**
 * Autogenerated catalog codes (K1-03 parte 2): reuses the same counter-table +
 * `SELECT ... FOR UPDATE` idiom as `allocateFolio` in quote-requests/service.ts,
 * with new sequence keys — `FolioSequence` is already generic, no migration needed.
 */
async function allocateCatalogCode(transaction: Prisma.TransactionClient, sequenceKey: string, prefix: string): Promise<string> {
  await transaction.folioSequence.upsert({ where: { key: sequenceKey }, update: {}, create: { key: sequenceKey, nextValue: 1 } });
  const rows = await transaction.$queryRaw<Array<{ nextValue: number }>>(Prisma.sql`
    SELECT "nextValue" FROM "folio_sequences" WHERE "key" = ${sequenceKey} FOR UPDATE
  `);
  const sequence = rows[0]?.nextValue;
  if (!sequence || sequence > 999_999) throw new Error('Catalog code sequence is unavailable.');
  await transaction.folioSequence.update({ where: { key: sequenceKey }, data: { nextValue: { increment: 1 } } });
...
export async function createCatalogCategory(actor: Actor, input: { code?: string; name: string; description?: string | null; sortOrder?: number; parentId?: string | null }, dependencies: CatalogServiceDependencies = {}) {
  requireStaffPermission(actor, 'catalog.manage');
  const prisma = dependencies.prisma ?? getPrisma();
  const manualCode = input.code !== undefined && input.code !== null && input.code !== '';
  const code = manualCode ? normalizeCode(input.code!, 'La clave de categoría no es válida.') : null;
  ...
  const parentId = input.parentId ? requireUuid(input.parentId, 'La categoría padre no es válida.') : null;
  ...
  try {
    return await prisma.$transaction(async (transaction) => {
      ...
      const category = await transaction.catalogCategory.create({ data: { code: finalCode, name, description, sortOrder, parentId } });
      await audit(transaction, actor, 'catalog.category.created', 'catalog_category', category.id, { code: finalCode, name, codeSource: manualCode ? 'MANUAL' : 'AUTO' });
      await outbox(transaction, 'CATALOG.CATEGORY_CREATED', 'CATALOG_CATEGORY', category.id, { categoryId: category.id, code: finalCode });
      return category;
    });
  } catch (error) {
...
export async function updateCatalogCategory(actor: Actor, categoryId: string, input: { name?: string; description?: string | null; sortOrder?: number; status?: CatalogStatus; parentId?: string | null }, dependencies: CatalogServiceDependencies = {}) {
  requireStaffPermission(actor, 'catalog.manage');
  const prisma = dependencies.prisma ?? getPrisma();
  ...
  if (input.parentId !== undefined) {
    ...
    if (newParentId) {
      ...
      while (cursor) {
        if (cursor === id) throw new AppError('VALIDATION_ERROR', 'Esa asignación crearía un ciclo entre categorías.', 400);
        if (seen.has(cursor)) break;
        seen.add(cursor);
        cursor = (await prisma.catalogCategory.findUnique({ where: { id: cursor }, select: { parentId: true } }))?.parentId ?? null;
      }
    }
...
export async function createCatalogItem(actor: Actor, input: { code?: string; name: string; description?: string | null; unit: string; categoryId?: string | null }, dependencies: CatalogServiceDependencies = {}) {
  ...
  const prisma = dependencies.prisma ?? getPrisma();
  const manualCode = input.code !== undefined && input.code !== null && input.code !== '';
  const code = manualCode ? normalizeCode(input.code!, 'La clave de concepto no es válida.') : null;
  ...
  try {
    return await prisma.$transaction(async (transaction) => {
      ...
      const finalCode = code ?? await allocateCatalogCode(transaction, 'catalog_item', 'ITEM');
      const item = await transaction.catalogItem.create({ data: { code: finalCode, name, description, unit, categoryId } });
      await audit(transaction, actor, 'catalog.item.created', 'catalog_item', item.id, { code: finalCode, name, codeSource: manualCode ? 'MANUAL' : 'AUTO' });
      await outbox(transaction, 'CATALOG.ITEM_CREATED', 'CATALOG_ITEM', item.id, { itemId: item.id, code: finalCode });
      return item;
    });
  } catch (error) {
