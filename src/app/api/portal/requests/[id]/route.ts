...
export async function GET(request: NextRequest, context: RouteContext) {
  ...
  try {
    ...
    const { id: requestIdValue } = await context.params;
    return NextResponse.json(await getCustomerQuoteRequest(actor, requestIdValue), { headers: { 'cache-control': 'no-store' } });
  } catch (error) {
