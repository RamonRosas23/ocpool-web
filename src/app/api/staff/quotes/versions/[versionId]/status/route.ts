...
import { publishQuoteVersion, transitionQuoteVersion } from '@/server/modules/quotes/service';
...
export async function POST(request: NextRequest, context: RouteContext) {
  ...
  try {
    ...
    const result = body.toStatus === 'ENVIADA'
      ? await publishQuoteVersion(actor, versionId)
      : await transitionQuoteVersion(actor, versionId, body.toStatus);
