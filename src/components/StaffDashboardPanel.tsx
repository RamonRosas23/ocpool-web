...
export default function StaffDashboardPanel() {
  ...
  return <main className="staff-shell analytics-shell">
    <header className="staff-header"><Link className="staff-brand" href="/" aria-label="OCPOOL, volver al sitio público"><span>OCPOOL</span><small>Operaciones comerciales</small></Link><nav className="analytics-nav" aria-label="Navegación de operaciones"><Link href="/staff">Dashboard</Link><Link href="/staff/requests">Solicitudes</Link><Link href="/staff/quotes">Cotizaciones</Link><Link href="/staff/notifications">Notificaciones</Link></nav><div className="staff-header__context"><span className="staff-header__pulse" aria-hidden="true" /> Lectura operativa</div></header>
    <div className="staff-content analytics-content" aria-busy={loading}>
      ...
      <section className="analytics-controls" aria-label="Controles del periodo"><div className="analytics-presets"><span>Vista rápida</span><button className={selectedPreset === '7' ? 'is-selected' : ''} type="button" onClick={() => applyPreset(7)} disabled={!draftTo}>7 días</button><button className={selectedPreset === '30' ? 'is-selected' : ''} type="button" onClick={() => applyPreset(30)} disabled={!draftTo}>30 días</button><button className={selectedPreset === '90' ? 'is-selected' : ''} type="button" onClick={() => applyPreset(90)} disabled={!draftTo}>90 días</button></div><div className="analytics-date-form"><label><span>Desde</span><input type="date" value={draftFrom} onChange={(event) => { rangeEdited.current = true; setDraftFrom(event.target.value); }} /></label><label><span>Hasta</span><input type="date" value={draftTo} onChange={(event) => { rangeEdited.current = true; setDraftTo(event.target.value); }} /></label><button className="staff-button staff-button--dark" type="button" onClick={applyCustomRange} disabled={!draftFrom || !draftTo || loading}>Aplicar periodo</button></div></section>
      ...
      {dashboard && <>
        ...
        <section className="analytics-workload" aria-labelledby="analytics-workload-title"><div className="analytics-workload__heading"><div><p className="staff-section-label">Distribución de trabajo</p><h2 id="analytics-workload-title">Carga por responsable</h2><p>La identidad visible se limita al personal operativo. Las muestras pequeñas permanecen protegidas.</p></div><span className="analytics-workload__scope">{dashboard.meta.scope === 'global' ? 'Vista global' : 'Tu alcance'}</span></div>{dashboard.workload.length === 0 ? <div className="analytics-empty"><strong>Sin carga abierta visible.</strong><p>Las solicitudes y cotizaciones activas aparecerán cuando exista actividad asignada.</p></div> : <div className="analytics-workload-table" role="table" aria-label="Carga por responsable"><div className="analytics-workload-table__row analytics-workload-table__row--head" role="row"><span>Responsable</span><span>Solicitudes activas</span><span>Borradores</span><span>Más antigua</span></div>{dashboard.workload.map((row) => <div className="analytics-workload-table__row" role="row" key={row.actorKey}><strong>{row.displayName}</strong>{row.suppressed ? <><span className="is-suppressed">Muestra protegida</span><span className="is-suppressed">Muestra protegida</span><span className="is-suppressed">Muestra protegida</span></> : <><span>{formatInteger(row.activeRequests)}</span><span>{formatInteger(row.draftQuotes)}</span><span>{formatDate(row.oldestOpenAt, dashboard.meta.timezone)}</span></>}</div>)}</div>}</section>
      </>}
    </div>
  </main>;
}
