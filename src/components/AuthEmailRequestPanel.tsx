...
export default function AuthEmailRequestPanel({ kind }: { kind: RequestKind }) {
  ...
  const submit = async (event: FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      setNotice('Si el correo está asociado a una cuenta activa, recibirás un enlace en unos minutos.');
    } catch (caught) {
  ...
  return <main className={`auth-shell auth-shell--${customer ? 'client' : 'recovery'}`}>
    ...
    <section className="auth-panel" aria-labelledby="auth-request-title">
      <div className="auth-panel__top"><p className="auth-kicker">{customer ? 'Entrada de cliente' : 'Recuperación'}</p><Link href={customer ? '/login' : '/login'} className="auth-panel__back">Volver al acceso</Link></div>
      <div className="auth-panel__body">
        <h2 id="auth-request-title">{customer ? 'Accede a tu portal' : 'Recupera tu acceso'}</h2>
        <p className="auth-panel__intro">{customer ? 'Escribe el correo asociado a tu expediente y te enviaremos un enlace seguro.' : 'Te enviaremos un enlace temporal para definir una nueva contraseña.'}</p>
        <form className="auth-form" onSubmit={submit} noValidate>
          ...
          <button className="auth-submit" type="submit" disabled={busy}>{busy ? 'Enviando…' : customer ? 'Enviar enlace' : 'Enviar solicitud'}</button>
        </form>
      </div>
