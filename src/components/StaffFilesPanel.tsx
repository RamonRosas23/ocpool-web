...
export default function StaffFilesPanel({ requestId, capabilities = DEFAULT_CAPABILITIES }: { requestId: string; capabilities?: StaffFilesCapabilities }) {
  ...
  const loadFiles = useCallback(async () => {
    ...
    setLoading(true);
    try {
      const response = await fetch(endpoint(requestId, '?limit=100'), { credentials: 'include', cache: 'no-store' });
  ...
  const upload = async () => {
    ...
    try {
      ...
      const reserveResponse = await fetch(endpoint(requestId), {
        ...
        body: JSON.stringify({ originalFileName: selectedFile.name, contentType: selectedFile.type || 'application/octet-stream', byteSize: selectedFile.size, category, visibility, idempotencyKey: `staff-${window.crypto.randomUUID()}` }),
      });
      const reservation = await readResponse<ReserveResponse>(reserveResponse);
      if (!reservation.uploadUrl) { await loadFiles(); return; }
      setUploadState('Subiendo archivo…');
      const uploadResponse = await fetch(reservation.uploadUrl, { method: 'PUT', headers: { 'content-type': selectedFile.type || 'application/octet-stream' }, body: selectedFile });
  ...
  return <section className="staff-files" aria-labelledby={headingId}>
    ...
    {showUpload && capabilities.filesUpload && <div className="staff-files__upload"><label><span>Archivo</span><input type="file" accept=".pdf,.jpg,.jpeg,.png,.webp,application/pdf,image/jpeg,image/png,image/webp" onChange={selectFile} disabled={uploading} /></label><label><span>Visibilidad</span><select value={visibility} onChange={(event) => setVisibility(event.target.value as FileVisibility)} disabled={uploading}><option value="CUSTOMER">Compartido con cliente</option><option value="INTERNAL">Sólo equipo</option></select></label><label><span>Categoría</span><select value={category} onChange={(event) => setCategory(event.target.value as FileCategory)} disabled={uploading}>{availableCategories.map((option) => <option key={option} value={option}>{categoryLabel(option)}</option>)}</select></label><div className="staff-files__upload-foot"><span>{selectedFile ? selectedFile.name : 'Ningún archivo seleccionado'}</span><button type="button" className="staff-button staff-button--dark" disabled={!selectedFile || uploading} onClick={() => void upload()}>{uploading ? 'Cargando…' : 'Cargar archivo'}</button></div></div>}
    {uploadState && <p className="staff-files__progress" role="status" aria-live="polite">{uploadState}</p>}
