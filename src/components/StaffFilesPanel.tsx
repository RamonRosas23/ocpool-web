...
import { fileCategoryLabel, fileStatusIcon, fileStatusLabel } from '@/lib/labels';
...
function FileStatusBadge({ file }: { file: FileItem }) {
  ...
  return <span className={`staff-file__status staff-file__status--${file.status.toLowerCase()}`}><StatusIcon size={13} aria-hidden="true" />{fileStatusLabel(file)}</span>;
}
