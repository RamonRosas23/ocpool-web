...
function RestrictedAudit() {
  return <main className="staff-shell staff-shell--restricted"><section className="staff-empty"><span className="staff-empty__mark">OC</span><p className="staff-kicker">Área interna</p><h1>Acceso restringido.</h1><p>Necesitas una cuenta de empleado con permiso de auditoría para consultar esta operación.</p><div className="staff-empty__actions"><Link className="staff-button staff-button--dark" href="/login">Iniciar sesión</Link><Link className="staff-empty__link" href="/">Volver al sitio</Link></div></section></main>;
}
...
export default function StaffAuditPanel() {
  ...
  return <main className="staff-shell audit-shell">
    <header className="staff-header"><Link className="staff-brand" href="/" aria-label="OCPOOL, volver al sitio público"><span>OCPOOL</span><small>Operaciones comerciales</small></Link><nav className="audit-nav" aria-label="Navegación de operaciones"><Link href="/staff">Dashboard</Link><Link href="/staff/requests">Solicitudes</Link><Link href="/staff/quotes">Cotizaciones</Link><Link href="/staff/notifications">Notificaciones</Link><Link className="is-current" href="/staff/audit" aria-current="page">Auditoría</Link></nav><div className="staff-header__context"><span className="staff-header__pulse" aria-hidden="true" /> Trazabilidad protegida</div></header>
    <div className="staff-content audit-content" aria-busy={loading}>
      ...
      <section className="audit-toolbar" aria-label="Filtros de auditoría"><label htmlFor="audit-from"><span>Desde</span><input id="audit-from" type="date" value={draftFrom} onChange={(event) => setDraftFrom(event.target.value)} /></label><label htmlFor="audit-to"><span>Hasta</span><input id="audit-to" type="date" value={draftTo} onChange={(event) => setDraftTo(event.target.value)} /></label><label htmlFor="audit-category"><span>Filtrar por categoría</span><select id="audit-category" value={draftCategory} onChange={(event) => setDraftCategory(event.target.value as Category)}><option value="">Toda la actividad operativa</option>{CATEGORY_OPTIONS.filter((category): category is Exclude<Category, ''> => category !== '' && (category !== 'security' || Boolean(capabilities.auditSecurityRead))).map((category) => <option key={category} value={category}>{CATEGORY_LABELS[category]}</option>)}</select></label><label htmlFor="audit-outcome"><span>Filtrar por resultado</span><select id="audit-outcome" value={draftOutcome} onChange={(event) => setDraftOutcome(event.target.value as Outcome)}><option value="">Todos los resultados</option>{OUTCOME_OPTIONS.filter((outcome): outcome is Exclude<Outcome, ''> => outcome !== '').map((outcome) => <option key={outcome} value={outcome}>{OUTCOME_LABELS[outcome]}</option>)}</select></label><button className="staff-button staff-button--dark" type="button" onClick={applyFilters} disabled={loading || !draftFrom || !draftTo}>Aplicar filtros</button></section>
